...
struct VS_INPUT
{
   ...
   float3 mTangent : BINORMAL;
   float3 mBinormal : TANGENT;
};
...
VS_OUTPUT ColorConversion_EnvironmentMapping_Vertex_Shader_vs_main( VS_INPUT Input )
{
   ...
   Output.T = mul(Input.mNormal, (float3x3)gWorldMatrix);
...
float4 ColorConversion_EnvironmentMapping_Pixel_Shader_ps_main(PS_INPUT Input) : COLOR
{
   ...
   float ambient = float3(0.1f, 0.1f, 01.f) * albedo;
 //  return float4(environment, 1);
   return float4(ambient + diffuse + specular + environment * 0.6f, 1);
}
//--------------------------------------------------------------//
// NoEffect
//--------------------------------------------------------------//
...
sampler2D ColorConversion_Grayscale_Pixel_Shader_SceneSampler = sampler_state
{
   Texture = (SceneTexture_Tex);
};
...
{
   float4 tex = tex2D(ColorConversion_Grayscale_Pixel_Shader_SceneSampler, Input.mUV);
...
sampler2D ColorConversion_Sepia_Pixel_Shader_SceneSampler = sampler_state
{
   Texture = (SceneTexture_Tex);
};
...
{
   float4 tex = tex2D(ColorConversion_Sepia_Pixel_Shader_SceneSampler, Input.mUV);
